...
def prepare_snli_batches(args):
    ...
    debug_train = pre + 'tiny_clean_train.jsonl'
    ...
    if not os.path.exists(os.path.join(data_dir, "snli_1.0_" + test_path)):
        remove_unk('test')

    if not os.path.exists(os.path.join(data_dir, "snli_1.0_" + validation_path)):
        remove_unk('dev')
    ...
    if args.debug:
        ...
        train, dev, test = datasets.SNLI.splits(inputs, answers, transitions,
            train=debug_train, validation=debug_train, test=debug_train)
    else:
